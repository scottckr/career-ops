...
// ── Company filter ───────────────────────────────────────────────────
// Optional. If `company_filter` is absent from portals.yml, every company
// passes. Blocks a company from being scanned at all (blocked_names: exact
// match, blocked_keywords: substring match) — for companies you never want
// to see regardless of role/location, distinct from data/blacklist.md
// (user's own do-not-apply decisions logged after the fact).
...
async function main() {
  ...
  function resolveEntries(entries, { isBoard = false } = {}) {
    for (const entry of entries) {
      ...
      if (!companyFilter(entry.name)) continue;
  ...
  if (config.company_filter || totalFilteredCompany > 0) {
