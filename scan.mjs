...
function buildCompanyFilter(companyFilter) {
  const blockedNames = (companyFilter?.blocked_names || []).map((k) =>
    k.toLowerCase(),
  );
  const blockedKeywords = (companyFilter?.blocked_keywords || []).map((k) =>
    k.toLowerCase(),
  );
  ...
  function compiledPositiveMatchers(positiveList) {
    if (compiledPositiveCache.has(positiveList))
      return compiledPositiveCache.get(positiveList);
  ...
  export function matchedTitleKeywords(title, titleFilter) {
    const raw = Array.isArray(titleFilter?.positive)
  ...
  export function buildContentFilter(contentFilter) {
    ...
    if (
      contentFilter.by_title_keyword &&
      typeof contentFilter.by_title_keyword === "object" &&
      !Array.isArray(contentFilter.by_title_keyword)
    ) {
      ...
      if (overrides.length > 0) {
        return overrides.some((rule) => {
          if (
            ...
            rule.negative.some((k) => lower.includes(k))
          )
            return false;
  ...
  export function buildSalaryFilter(salaryFilter) {
    ...
    if (!Number.isFinite(min) || !Number.isFinite(max) || min < 0 || max < 0) {
      console.error(
        "Warning: salary_filter.min/max must be non-negative numbers — salary filter disabled",
      );
      return () => true;
    }
    if (max > 0 && min > max) {
      console.error(
        "Warning: salary_filter.min cannot exceed salary_filter.max — salary filter disabled",
      );
      return () => true;
    }
  ...
  export function companyMatch(jobCompany, windowCompany) {
    ...
    const regex1 = new RegExp(
      "\\b" + c2WithSpaces.replace(/[-\/\\^$*+?.()|[\]{}]/g, "\\$&") + "\\b",
    );
    const regex2 = new RegExp(
      "\\b" + c1WithSpaces.replace(/[-\/\\^$*+?.()|[\]{}]/g, "\\$&") + "\\b",
    );
  ...
  export function loadReApplyWindows(profilePath = PROFILE_PATH) {
    ...
    try {
      ...
      for (const [company, win] of Object.entries(windows)) {
        ...
        if (
          typeof lastApplyDate !== "string" ||
          !/^\d{4}-\d{2}-\d{2}$/.test(lastApplyDate)
        )
          continue;
        ...
        if (
          sameRoleDays !== undefined &&
          (!Number.isInteger(sameRoleDays) || sameRoleDays < 0)
        )
          continue;

        if (win.applied_to !== undefined && !Array.isArray(win.applied_to))
          continue;
        if (
          win.applied_to !== undefined &&
          win.applied_to.some((x) => typeof x !== "string")
        )
          continue;

        if (
          win.cross_role_bucket !== undefined &&
          typeof win.cross_role_bucket !== "string"
        )
          continue;
        ...
        if (companyMatch(jobCompany, windowCompany)) {
          ...
          if (Array.isArray(window.applied_to)) {
            ...
            if (matchesApplied) {
              return {
                skip: true,
                reason: `cooldown:${windowCompany}:${cooldownUntil}`,
                cooldownUntil,
              };
            }
          }
          ...
          if (window.cross_role_bucket) {
            ...
            const matchesBucket = bucketKeywords.some((kw) => {
              if (kw === "em") {
                return (
                  /\bem\b/i.test(jobTitleLower) ||
                  jobTitleLower.includes("engineering manager")
                );
              }
  ...
  export function shouldDedupScanHistoryRow(
    { firstSeen, status = "added" },
    {
  ...
  function isRoleLocationSuffix(tag) {
    ...
    if (
      parts.length > 1 &&
      parts.every((part) => ROLE_LOCATION_SUFFIXES.has(part))
    )
      return true;
    ...
    for (const remote of ROLE_REMOTE_SUFFIXES) {
      ...
      if (
        ...
        ROLE_LOCATION_SUFFIXES.has(normalized.slice(prefix.length))
      ) {
        return true;
      }
    }
   ...
   */
  export function companyRoleDedupKey(
    company,
    role,
   ...
   */
  export function collectSeenCompanyRoles(
    sources = {},
    policy = {},
    canonicalize = defaultCompanyNormalizer,
  ) {
    ...
    for (const line of scanHistoryText.split("\n").slice(1)) {
      // skip header
      const [url, firstSeen, , title, company, status = "added"] =
        line.split("\t");
      if (!url) continue;
    ...
    for (const match of pipelineText.matchAll(
      /^- \[[ x]\]\s+\S+\s*\|\s*([^|\n]+?)\s*\|\s*([^|\n]+?)\s*(?:\|[^\n]*)?$/gm,
    )) {
      add(match[1], match[2]);
    }
  ...
  export function formatCompensation(salary) {
    ...
    const num = (n) =>
      Number.isFinite(n) && n > 0 ? String(Math.round(n)) : null;
    const lo = num(salary.min);
    const hi = num(salary.max);
    const range = lo && hi && lo !== hi ? `${lo}-${hi}` : lo || hi || "";
    if (!range) return "";
    const currency =
      typeof salary.currency === "string" ? salary.currency.trim() : "";
  ...
  export function trustIsFlagged(offer) {
    return (
      ...
      Number.isFinite(offer.trustScore) &&
  ...
  export function formatTrustSegment(offer) {
    ...
    const body = flags.length
      ? `${offer.trustScore} ${flags.join(",")}`
      : String(offer.trustScore);
  ...
  export function formatPipelineOffer(offer) {
    ...
    const location =
      typeof offer.location === "string"
        ? sanitizeMarkdownField(offer.location)
        : "";
    ...
    const note =
      typeof offer.note === "string" ? sanitizeMarkdownField(offer.note) : "";
...
`;
  ...
  export function appendToScanHistory(offers, date, status = "added") {
    ...
    const lines =
      offers.map((o) => formatScanHistoryRow(o, date, status)).join("\n") +
      "\n";
  ...
  const PORTAL_HEALTH_PATH = path.join(
    path.dirname(fileURLToPath(import.meta.url)),
    "data",
    "portal-health.tsv",
  );
  ...
  export function appendPortalHealth(
    healthRecords,
    filePath = PORTAL_HEALTH_PATH,
  ) {
  ...
  async function verifyOffers(
    offers,
    { headedFallback = false, throttleBaseMs = 0, rediscover = false } = {},
  ) {
    ...
    try {
      ...
      for (let i = 0; i < offers.length; i++) {
        ...
        const { result, code, reason } = headed
          ? await checkUrlLivenessWithFallback(page, offer.url, {
              getHeadedPage,
            })
        ...
        if (result === "expired") {
          ...
          if (
            ...
            if (newUrl) {
              ...
              const recheck = headed
                ? await checkUrlLivenessWithFallback(page, newUrl, {
                    getHeadedPage,
                  })
              ...
              if (recheck.result === "active") {
                migrated.push({
                  ...offer,
                  url: newUrl,
                  previousUrl: offer.url,
                });
                console.log(
                  `  🔄 migrated  ${offer.company} | ${offer.title} → ${newUrl}`,
                );
                continue;
              }
            }
          ...
          console.log(
            `  ❌ expired   ${offer.company} | ${offer.title} (${reason})`,
          );
          ...
          console.log(
            `  ⛔ invalid   ${offer.company} | ${offer.title} (${reason})`,
          );
          ...
          console.log(
            `  ⚠️ no-apply  ${offer.company} | ${offer.title} (${reason})`,
          );
        } else {
          ...
          console.log(
            `  ${icon} ${result.padEnd(9)} ${offer.company} | ${offer.title}`,
          );
        }

        const wait =
          i < offers.length - 1 ? jitteredDelayMs(throttleBaseMs) : 0;
  ...
  async function main() {
    ...
    const throttleArg = args.find(
      (a) => a === "--throttle" || a.startsWith("--throttle="),
    );
    const throttleBaseMs = throttleArg
      ? Number(throttleArg.split("=")[1]) || 5000
      : 0;
    ...
    const postedAfter =
      postedAfterFlag !== -1 ? args[postedAfterFlag + 1] : null;
    const postedBeforeFlag = args.indexOf("--posted-before");
    const postedBefore =
      postedBeforeFlag !== -1 ? args[postedBeforeFlag + 1] : null;
    if (postedAfter != null && !isValidIsoDate(postedAfter)) {
      console.error(
        `Error: --posted-after expects YYYY-MM-DD, got "${postedAfter}"`,
      );
      process.exit(1);
    }
    if (postedBefore != null && !isValidIsoDate(postedBefore)) {
      console.error(
        `Error: --posted-before expects YYYY-MM-DD, got "${postedBefore}"`,
      );
      process.exit(1);
    }
    ...
    const visaEnabled =
      Boolean(config.visa_filter) && config.visa_filter.enabled !== false;
    ...
    function resolveEntries(entries, { isBoard = false } = {}) {
      for (const entry of entries) {
        ...
        if (typeof entry.name !== "string" || !entry.name.trim()) {
          console.error(
            `⚠️  Skipping entry — missing or non-string 'name' field: ${JSON.stringify(entry)}`,
          );
          continue;
        }
        if (filterCompany && !entry.name.toLowerCase().includes(filterCompany))
          continue;
    ...
    const localParserCount = targets.filter(
      (t) => t._provider.id === "local-parser",
    ).length;
    ...
    const canonicalizeCompany = buildCompanyCanonicalizer(
    ...
    const tasks = targets.map((company) => async () => {
      ...
      let sourceName =
        provider.id === "local-parser" ? "local-parser" : `${provider.id}-api`;
      try {
        let jobs;
        try {
          ...
          const fallback = resolveProvider(company, providers, {
            skipIds: ["local-parser"],
          });
        ...
        for (const job of jobs) {
          ...
          if (blacklist.size > 0) {
            const blEntry = blacklist.get(
              normalizeCompany(job.company || company.name || ""),
            );
            if (blEntry) {
              ...
              job.note =
                typeof job.note === "string" && job.note.trim()
                  ? `${label} — ${job.note}`
                  : label;
            }
          }
          ...
          if (
            !contentFilter(
              job.description,
              matchedTitleKeywords(job.title, config.title_filter),
            )
          ) {
    ...
    const crossListings = findCrossListings(
      verifiedOffers,
    ...
    if (summaryBoards > 0)
      console.log(`Job boards scanned:    ${summaryBoards}`);
    ...
    if (postedAfter || postedBefore) {
      console.log(
        `Filtered by posted date: ${totalFilteredPostedDate} removed`,
      );
    }
    ...
    if (blacklist.size > 0) {
      if (includeBlacklisted) {
        console.log(
          `Blacklisted:           ${annotatedBlacklisted} let through annotated (--include-blacklisted)`,
        );
      } else {
        console.log(
          `Blacklisted:           ${totalFilteredBlacklist} skipped (blacklist)`,
        );
      }
    }
    if (crossListings.length > 0) {
      ...
      for (const { offer, row, score } of crossListings) {
        ...
        console.log(
          `    ≈ ${Math.round(score * 100)}% of ${row.company} — ${row.title} (seen ${row.dateStr})`,
        );
        console.log(`    ${offer.url}`);
        console.log(`    vs ${row.url}`);
      }
      console.log(
        `  If one side is an agency, apply through ONE channel only — a double submission burns both (#1596).`,
      );
    }
    if (historyPolicy.recheckAfterDays != null) {
      console.log(
        `Recheck eligible:      ${seenUrlState.recheckEligible} old scan-history URL(s)`,
      );
    }
    if (verify) {
    ...
    if (
      config.trust_filter &&
      config.trust_filter.enabled !== false &&
      verifiedOffers.length > 0
    ) {
      const trustHigh = verifiedOffers.filter(
        (o) => o.trustLevel === "high",
      ).length;
      const trustMedium = verifiedOffers.filter(
        (o) => o.trustLevel === "medium",
      ).length;
      const trustLow = verifiedOffers.filter(
        (o) => o.trustLevel === "low",
      ).length;
      console.log(
        `Trust validation:      ${trustHigh} high, ${trustMedium} medium, ${trustLow} low`,
      );
      // Flag breakdown
    ...
    if (agentHandoff.length > 0) {
      console.log(
        `Agent/WebSearch handoff: ${agentHandoff.length} compan${agentHandoff.length === 1 ? "y" : "ies"} not handled by zero-token providers`,
      );
      ...
      if (agentHandoff.length > 25) {
        console.log(
          `  … ${agentHandoff.length - 25} more omitted; narrow with --company or inspect portals.yml`,
        );
      }
    }
    ...
    const otherErrors = errors.filter(
      (e) => e.kind !== "slug_gone" && e.kind !== "network",
    );
    ...
    for (const t of targets) {
      const isUnreachable = unreachableTargets.some(
        (e) => e.company === t.name,
      );
    ...
    const currentStreaks = computeConsecutiveFailures([
      ...pastHealth,
      ...healthRecords,
    ]);
    ...
    for (const e of [...unreachableTargets, ...networkTargets]) {
      ...
      if (streak >= STREAK_THRESHOLD) {
        if (!persistentlyDead.includes(e.company))
          persistentlyDead.push(e.company);
      } else {
        if (e.kind === "slug_gone") {
          if (!newlyDeadSlug.some((x) => x.company === e.company))
            newlyDeadSlug.push(e);
        } else {
    ...
    if (persistentlyDead.length > 0) {
      ...
      console.log(
        `   Run: node verify-portals.mjs to check if the ATS migrated, or update their board slugs.`,
      );
    }
    ...
    if (emptyTargets.length > 0) {
      console.log(
        `🟡 ${emptyTargets.length} target(s) live but empty: ${emptyTargets.join(", ")}`,
      );
    }
    ...
    if (verifiedOffers.length > 0) {
      ...
      for (const o of verifiedOffers) {
        const trustSuffix =
          o.trustScore != null && o.trustScore < 100
            ? ` [Trust: ${o.trustScore}/100${o.trustFlags?.length ? " — " + o.trustFlags.join(", ") : ""}]`
            : "";
        const blacklistSuffix = o.blacklisted
          ? " [BLACKLISTED — on your do-not-apply list]"
          : "";
        console.log(
          `  + ${o.company} | ${o.title} | ${o.location || "N/A"}${trustSuffix}${blacklistSuffix}`,
        );
      }
      if (dryRun) {
    ...
    if (
      ...
      try {
        writeFileSync(".manifesto-noted", new Date().toISOString() + "\n");
      } catch {
        /* best-effort */
      }
    }
